...
def draw_spot_cone(light, target_pos: Vector, cam_pos: Vector, region=None, rv3d=None, batch_lines=None, batch_shapes=None):
    ...
    base_distance = 1.0
    radius = max(0.0, getattr(light.data, "shadow_soft_size", 0.0))
    max_angle = math.pi
    angle_clamped = max(0.0, min(angle, max_angle))
    angle_factor = (max_angle - angle_clamped) / max_angle
    distance = (base_distance + radius) * angle_factor
